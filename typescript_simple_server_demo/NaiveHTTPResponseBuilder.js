...
function NaiveHTTPResponseBuilder(request, response) {
    ...
    if (url === "/" || url === "/home" || url === "/index") {
        ...
        fs_1.createReadStream(__dirname + "/index.html", "utf8")
            .pipe(response)
        ...
        response.end("redirecting to your worst nightmare", "utf8", () => console.log("response has been sent"));
    }
        ...
        // you can manually listen to the 'finish' event on the piped stream
        // otherwise response will be ended by pipe automatically
        fs_1.createReadStream(__dirname + "/lorem_ipsum.txt", "utf8").pipe(response);
    }
        ...
        response.end(JSON.stringify(myObj), () => console.log(`JSON : ${myObj} has been sent to ${request.url}`));
    }
    else {
        response.writeHead(404, { "Content-Type": "text/html" });
        fs_1.createReadStream(__dirname + "/404.html", "utf8").pipe(response);
    }
}
exports.NaiveHTTPResponseBuilder = NaiveHTTPResponseBuilder;
//# sourceMappingURL=NaiveHTTPResponseBuilder.js.map